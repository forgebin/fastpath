...
function Trajectory:CalculateSingle(base_part : BasePart, x0 : Vector3, v0  : Vector3, ignoreList)
	local t = 0
	...
	repeat
		...
		self.TimeStep = 1/(v0.Magnitude)
		
		local quick_params = RaycastParams.new()
		...
		if raycast then
			...
			hit, pos, normal, material = false, p0 + (p1 - p0), nil, nil
		end
...
function Trajectory:Cast(base_part, material, ignoreList)
	local bounce = 0
	...
	local x0 = base_part.Position
	local v0 = base_part.AssemblyLinearVelocity
	
	while (v0:Dot(v0) >= speed2 and bounce <= self.MaxBounce) do
		t, normal, pB = self:CalculateSingle(base_part, x0, v0, ignoreList)
		if (t) then
